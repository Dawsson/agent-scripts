---
summary: 'Agent handoff checklist.'
read_when:
...
4) Running processes: list tmux sessions/panes and how to attach:
   - Example: `tmux attach -t agent-shell` or `tmux capture-pane -p -J -t agent-shell:0.0 -S -200`
   - Note dev servers, hotline connections, tests, debuggers, background scripts.
...
Location: `.claude/commands/handoff.md` (repo-local) or `~/.claude/commands/handoff.md` (global).
